      ...
      success(res) {
        ...
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          uni.showToast({
            icon: "none",
            title:
              (res.data as requestData<T>).errorMessage?.errorCode ||
              "请求错误",
          });
          reject(res);
        }
...
export const requestRows = <T>(options: UniApp.RequestOptions) => {
  // 1. 返回 Promise 对象
  return new Promise<requestRowsData<T>>((resolve, reject) => {
    uni.request({
      ...options,
      // 响应成功
      success(res) {
        ...
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          // 2.1 提取核心数据 res.data
          resolve(res.data as requestRowsData<T>);
        } else if (res.statusCode === 401) {
          // 401错误  -> 清理用户信息，跳转到登录页
          const memberStore = useMemberStore();
          memberStore.clearProfile();
          ...
          uni.showToast({
            icon: "none",
            title:
              (res.data as requestRowsData<T>).errorMessage?.errorCode ||
              "请求错误",
          });
          reject(res);
        }

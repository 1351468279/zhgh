...
import { getProvincialPersonInfo, uploadFile, uploadForm } from "@/services/applyProvincial"
import { getUserInfo } from '@/services/applyProvincial'
import { getReviewStatus } from "@/services/applyUnion"
import { onShow } from '@dcloudio/uni-app';
import { nationsData, educationsData, politicsData, degreesData ,jobTitleData} from '@/composible/data'
import type { UniFilePicker, UniFilePickerOnDeleteEvent, UniFilePickerOnSelectEvent, UniFilePickerTempFile } from '@uni-helper/uni-ui-types';
...
<template>
  <view class="sanYuBox">
    ...
    <uni-forms :model="baseFormData" validateTrigger="blur" :rules="rules" label-width="80px" ref="customForm">
      ...
      <uni-forms-item required label="工作单位" :name="rulesName.unit">
        <uni-easyinput v-model="baseFormData.unit" type="text" placeholder="请输入工作单位" />
      </uni-forms-item>
      <uni-forms-item required label="荣誉称号" :name="rulesName.designation">
        <uni-easyinput v-model="baseFormData.designation" type="text" placeholder="请输入工作单位" />
      </uni-forms-item>
      <uni-forms-item required label="授奖部门" :name="rulesName.department">
        <uni-easyinput v-model="baseFormData.department" type="text" placeholder="请输入职位" />
      </uni-forms-item>
      <uni-forms-item required label="获奖时间" :name="rulesName.awardsTime">
          <uni-datetime-picker type="date" return-type="string" v-model="baseFormData.awardsTime" />
        </uni-forms-item>
      <uni-forms-item required label="上传先进事迹">
        <uni-file-picker :disabled="userInfo?.process != 0" :limit="9" file-mediatype="all" @select="onSelect"
          @delete="onDelete" file-extname="pdf" />
      </uni-forms-item>
      <bean> </bean>

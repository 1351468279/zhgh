...
import {
  checkFile,
  downloadFile,
  getTeamProvincialListApi,
  reportSanYu,
} from "@/services/applyProvincialTeam";
import type { getProvincialTeamListType } from "@/types/provincialTeam";
...
const onScrollTopLower = async () => {
  ...
  if (cardList.value.length < total.value) {
    ...
    // offset.value++
    const res = await getTeamProvincialListApi(getSanYuListParams.value);
...
const total = ref(0);
// 定义获取三育人列表请求分页参数
const getSanYuListParams = ref<getProvincialTeamListType>({
  stuTeam: {
    fs: 0,
  },
  pageVo: {
...
// 封装分页列表函数
const getSanYuList = async (data: getProvincialTeamListType) => {
  const res = await getTeamProvincialListApi(data);
  console.log("onShow");
  ...
  >
    ...
    >
      <view
        ...
        :key="item.id"
        @click.stop="handoff(item.id, index)"
      >
        {{ item.name }}
      </view>

...
import { getOrgName, getReviewInfo, getReviewList, getReviewStatus, passAllReview } from '@/services/applyUnion'
...
const reviewAll = async () => {
  ...
  // 刷新一下列表
  const res = await getReviewList({ fs: fs.value, pageVo: { limit: limit.value, offset: offset.value, sidx: sidx.value, sord: sord.value } })
...
<template>
  <view class="box">
    <view class="reviewOption">
      <view class="reviewing" :class="{ active: setReviewActive == 1 }" @click="changeReviewState(1)">待审核</view>
      <view class="reviewed" :class="{ active: setReviewActive == 2 }" @click="changeReviewState(2)">已审核</view>
    </view>
    <view class="shenheCard">
      <view class="card" v-for="(item) in reviewList" @click="onClick(item.id)">
        <view class="tittle">
          <view class="main" :class="{ await: setAwaitActive == 1 }">{{ setAwaitActive == 1 ? '待审核' : '已审核' }}</view>
    ...
    <view class="reviewAll">
      <button type="primary" @click="reviewAll" v-if="isAdmin === true && isShow == true">一键审核</button>
    </view>
  </view>
</template>
